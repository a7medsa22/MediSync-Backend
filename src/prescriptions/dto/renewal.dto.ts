...
export class PrescriptionRenewalResponseDto {
    @ApiProperty({ description: 'Unique identifier for the prescription renewal' })
    id!: string;
    ...
    requestedAt!: Date;
    @ApiProperty({ description: 'Date when the renewal request was responded to' })
    respondedAt?: Date;
    @ApiProperty({ description: 'ID of the user who responded to the renewal request' })
    respondedBy?: string;
