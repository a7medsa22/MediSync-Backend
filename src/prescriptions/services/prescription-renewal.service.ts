...
export class PrescriptionRenewalService {
   ...
   */
  async requestRenewal(userId: string, prescriptionId: string) {
    ...
    if (prescription.renewalCount >= prescription.maxRenewals) {
      throw new BadRequestException(
        'Maximum number of renewals reached for this prescription',
      );
    }
    ...
    if (pendingRenewal)
      throw new ConflictException('Renewal request already pending');
    ...
    this.eventEmitter.emit('notification.trigger', {
      ...
      data: {
        ...
        patientId,
        medicationName:
          prescription.prescriptionMedications[0]?.drugName || 'medication',
      },
    });
  ...
  async approveRenewal(
    ...
    const newPrescription = await this.prisma.$transaction(async (tx) => {
      ...
      const newExpiryDate = approveDto.newExpiryDate
        ...
        : TimeUtils.addDays(
            new Date(),
            parseInt(
              process.env.PRESCRIPTION_DEFAULT_VALIDITY_DAYS || '90',
              10,
            ),
          );
      ...
      const created = await tx.prescription.create({
        ...
        include: {
          patient: {
            select: {
              user: { select: { firstName: true, id: true, email: true } },
            },
          },
          doctor: {
            select: { user: { select: { firstName: true, lastName: true } } },
    ...
    this.eventEmitter.emit('notification.trigger', {
      ...
      data: {
        ...
        medicationName:
          originalPrescription.prescriptionMedications[0]?.drugName ||
          'medication',
      },
    });
    ...
    await this.prescriptionCache.invalidatePatientPrescriptions(
      originalPrescription.patientId,
    );
